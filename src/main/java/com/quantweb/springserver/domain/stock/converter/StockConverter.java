...
public class StockConverter {
    ...
    public static StockResponseDto toStockResponseDto(List<Stock> stocks, List<InvestmentSectorsPieChart> pieCharts){

        List<StockResponseDto.StockRatio> stockRatioList = stocks.stream()
                .map(st -> new StockResponseDto.StockRatio(st.getStockName(), st.getSector(), st.getPercentage()))
                .toList();

        List<StockResponseDto.InvestmentSectorsPieChart> charts = pieCharts.stream()
                .map(pc -> new StockResponseDto.InvestmentSectorsPieChart(pc.getSector(), pc.getPercentage()))
                .toList();
