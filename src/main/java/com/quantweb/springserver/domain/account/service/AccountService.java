...
public class AccountService {
  ...
  @Transactional
  public void syncKoreaInvestmentMockAccount(Long userId, KoreaInvestmentMockAccountRequest request) {
    User user =
  ...
  @Transactional
  public void updateKoreaInvestmentMockAccount(Long userId, KoreaInvestmentMockAccountRequest request) {

    Account account = accountRepository.findByUserId(userId)
            .orElseThrow(() -> new IllegalArgumentException("계좌가 없습니다"));

    account.updateKoreaInvestmentMockAccount(request.getAppKey(), request.getSecretKey(), request.getAccountNumber());
  }
