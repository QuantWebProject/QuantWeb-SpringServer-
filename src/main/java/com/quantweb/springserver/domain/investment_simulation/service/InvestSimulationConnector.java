...
public class InvestSimulationConnector {
  ...
  public FirstFastApiInvestSimulationResponse requestFirstInvestSimulation(
      User user, FirstInvestSimulationRequest request) {

    TechnicalAnalysisStrategy technicalAnalysisStrategy =
        setFirstTechnicalAnalysisStrategy(
            request.getTechnicalAnalysisStrategy().getTechnicalAnalysisStrategy());

    ValueInvestmentStrategy valueInvestmentStrategy =
        setOption(request.getValueInvestmentStrategy());

    FirstStrategySetup strategySetup =
        FirstStrategySetup.builder()
    ...
    HttpEntity<FirstFastApiInvestSimulationRequest> requestEntity =
        new HttpEntity<>(requestBody, headers);

    ResponseEntity<FirstFastApiInvestSimulationResponse> responseEntity =
        REST_TEMPLATE.exchange(
            FAST_API_URL + "/invest-simulation/first",
            HttpMethod.POST,
            requestEntity,
            FirstFastApiInvestSimulationResponse.class);

    return responseEntity.getBody();
  }

  public RenewFastApiInvestSimulationResponse requestRenewInvestSimulation(
      User user, RenewInvestSimulationRequest request, int initialAmount, float fee) {
    TechnicalAnalysisStrategy technicalAnalysisStrategy =
        setFirstTechnicalAnalysisStrategy(request.getTechnicalAnalysisStrategy());

    RenewStrategySetup strategySetup =
        RenewStrategySetup.builder()
            .strategyName(request.getStrategyName())
            .stockSelection(request.getStockSelection())
    ...
    HttpEntity<RenewFastApiInvestSimulationRequest> requestEntity =
        new HttpEntity<>(requestBody, headers);

    ResponseEntity<RenewFastApiInvestSimulationResponse> responseEntity =
        REST_TEMPLATE.exchange(
            FAST_API_URL + "/invest-simulation/renew",
            HttpMethod.POST,
            requestEntity,
            RenewFastApiInvestSimulationResponse.class);

    return responseEntity.getBody();
  }

  private FirstFastApiInvestSimulationRequest setFirstStrategyBody(
      User user, FirstStrategySetup strategySetup) {
  ...
  private RenewFastApiInvestSimulationRequest setRenewStrategyBody(
      User user, RenewStrategySetup strategySetup) {
  ...
  private TechnicalAnalysisStrategy setFirstTechnicalAnalysisStrategy(
      TechnicalAnalysisStrategy technicalAnalysisStrategy) {
