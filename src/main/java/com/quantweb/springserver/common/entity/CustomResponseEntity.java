...
public class CustomResponseEntity<T> extends HttpEntity<T> {
    ...
    public static <T> CustomResponseEntity<T> of(HttpStatus status,
        @Nullable MultiValueMap<String, String> headers, @Nullable T body) {
        if (headers == null) {
    ...
    private CustomResponseEntity(HttpStatus status, @Nullable MultiValueMap<String, String> headers,
        @Nullable T body) {
        super(body, headers);
